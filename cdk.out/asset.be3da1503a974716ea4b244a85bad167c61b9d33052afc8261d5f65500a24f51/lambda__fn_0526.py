...
def process_record(record, src_tag_prefix, dst_tag_prefix, dst_prefix):
    ...
    if source_addr and source_addr.strip() != "-":
        ...
        if src_metadata:
            ...
            if 'security_groups' in src_metadata:
                for idx, sg in enumerate(src_metadata['security_groups']):
                    record_dict[f"src-sg-{idx+1}-id"] = sg['GroupId']
                    record_dict[f"src-sg-{idx+1}-name"] = sg['GroupName']
    ...
    if destination_addr and destination_addr.strip() != "-":
        ...
        if dst_metadata:
            ...
            if 'security_groups' in dst_metadata:
                for idx, sg in enumerate(dst_metadata['security_groups']):
                    record_dict[f"dst-sg-{idx+1}-id"] = sg['GroupId']
                    record_dict[f"dst-sg-{idx+1}-name"] = sg['GroupName']

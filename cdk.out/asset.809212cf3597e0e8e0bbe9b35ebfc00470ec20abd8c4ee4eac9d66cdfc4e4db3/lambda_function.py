...
def process_network_interface(eni, ec2_client, table, current_time, ttl, stats):
    ...
    try:
        ...
        if 'PrivateIpAddress' in eni:
            ...
            if check_and_update_item(table, metadata):
                stats['updated_items'] += 1
...
def check_and_update_item(table, item):
    """Check if item exists and update it, or create new if it doesn't exist"""
    try:
        ...
        if response['Items']:
            ...
            table.put_item(
                Item=item,
                ConditionExpression='ip_address = :ip',
                ExpressionAttributeValues={
                    ':ip': item['ip_address']
                }
            )
        ...
        logger.error(f"Error updating/creating item for IP {item['ip_address']}: {str(e)}")
        return False

def cleanup_old_entries(table, hours_threshold=1):
...
def process_nat_gateway(nat_gw, table, current_time, ttl, stats):
    ...
    if nat_ip:
        ...
        if check_and_update_item(table, item):
            stats['updated_items'] += 1
            stats['resource_types']['nat_gateway'] = stats['resource_types'].get('nat_gateway', 0) + 1
        stats['processed_items'] += 1
        ...
        for az_mapping in lb.get('AvailabilityZones', []):
            for address in az_mapping.get('LoadBalancerAddresses', []):
                if 'IpAddress' in address:
                    ...
                    if check_and_update_item(table, item):
                        stats['updated_items'] += 1
                        stats['resource_types']['load_balancer'] = stats['resource_types'].get('load_balancer', 0) + 1
                    stats['processed_items'] += 1
    except Exception as e:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        stats['deleted_items'] = cleanup_old_entries(table)
        logger.info(f"Cleaned up {stats['deleted_items']} old entries")
        ...
        # Calculate execution time and prepare metrics
        ...
        logger.info(f"Successfully processed {stats['processed_items']} items, "
                   ...
                   f"deleted {stats['deleted_items']} old items, "

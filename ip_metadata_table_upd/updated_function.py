...
def lambda_handler(event, context):
    ...
    def clean_old_entries():
        """Clean duplicate entries keeping only the most recent one"""
        try:
            # Scan the table to get all items
            response = table.scan(
                ProjectionExpression='ip_address,last_updated,#ts',
                ExpressionAttributeNames={'#ts': 'timestamp'}
            )
            
            # Create a dictionary to store the latest entry for each IP
            ip_entries = {}
            
            # Process all items
            for item in response.get('Items', []):
                ip = item['ip_address']
                current_timestamp = item.get('last_updated')
                ...
                if ip not in ip_entries:
                    ...
                    # Compare timestamps and mark older entry for deletion
                    existing_timestamp = ip_entries[ip]['timestamp']
                    if current_timestamp > existing_timestamp:
                        ...
                        # Delete current item as it's older
    ...
    def store_ip_metadata(ip_address, metadata):
        ...
        try:
            ...
            # Clean any duplicate entries for this IP
            clean_old_entries()
